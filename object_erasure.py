...
parser.add_argument("--prompt_file", type=str, default="data/demo.csv", help="CSV file for prompts")
parser.add_argument("--demo_mode", action=argparse.BooleanOptionalAction, help="Set flag for demo mode (to run without querying the data file)")

args = parser.parse_args()

# -----------------------------------
# Define classes to get the neighbors
# -----------------------------------
...
if not args.demo_mode:
    ...
    def load_targets_and_neighbors(filename):
        ...
        for target in tqdm(erased_species, desc="Finding neighbors for all targets"):
            ...
            current_disallowed = set(target.replace('_', ' ').split()) # Add 'golden', 'retriever'

            # Query for concepts, requesting more than needed to account for filtering
            query_results = searcher.query(
                ...
                pop_log_thresh=2.0,
                min_popularity=100,
                disallowed_words=current_disallowed,
                n_images_per_concept=10
            )

            # Extract the top N valid neighbors and update the state
...
#-----------------------------------
# Define three-stage generation code
#-----------------------------------
...
def calculate_projection_matrix(target_word, manual_neighbors=None, beta=1.0, gamma=1.0):
    ...
    P_c = (I - (beta * P_F.to(DEVICE))) + gamma * torch.matmul(P_F.to(DEVICE), P_R.to(DEVICE))
...
#-----------------------------------------------
# Use the defined classes to generate new images
#-----------------------------------------------

beta = args.beta
gamma = args.gamma

config_file = args.prompt_file
...
base_path = os.path.join(name_only)
...
neighbors_file = "multi_neighbors.json"

if not os.path.exists(neighbors_file):
    if args.demo_mode:
        print("ERROR: Cannot load neighbors from data file in demo mode. Needs to have neighbors provided in the manual_neighbors.json file.")
    print(f"Generating target word and manual neighbors for {name_only}")
    neighbor_dict = load_targets_and_neighbors(config_file)
else:
...
# Used to erase the target concept
target_word = list(neighbor_dict.keys())[0] # Assume single-target scenario
manual_neighbors = neighbor_dict[target_word]
...
for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    filename_orig = f"{orig_folder}/orig_{safe_species}_{idx:04d}_seed{seed}_img0.png"
    filename_ours = f"{ours_folder}/ours_{safe_species}_{idx:04d}_seed{seed}_{concept_type}_img0.png"
    ...
    orig_img[0][0].save(filename_orig)
    new_img[0][0].save(filename_ours)